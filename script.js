...
// ========= Puzzle Gallery + Lightbox (safe init) =========
...
(function () {
  ...
  function initPuzzleGallery() {
    ...
    // Guard: ако вече сме вързали listener-и за този grid, не повтаряме
    ...
    // Stagger delay (ако ползваш CSS анимация с --d)
    items.forEach((b, i) => b.style.setProperty("--d", `${i * 45}ms`));

    // Списък със снимките (data-full има приоритет)
    const images = items
    ...
    if (!window.__puzzleKeyBound) {
      ...
      window.addEventListener(
        ...
        (e) => {
          ...
          // Важно: ползваме текущото index/images от активната инстанция
          // Ако имаш само една галерия, това е ок.
          // При много галерии - ще трябва state по-сложен.
  ...
  // 1) първоначално
  document.addEventListener("DOMContentLoaded", initPuzzleGallery);

  // 2) ако имаш SPA/вътрешна навигация
  window.addEventListener("hashchange", initPuzzleGallery);
  window.addEventListener("popstate", initPuzzleGallery);

  // 3) fallback: пробвай още веднъж след load (за случаи с късно рендерване)
  window.addEventListener("load", initPuzzleGallery);

  // 4) ако DOM се променя динамично (инжектираш секции) - лек, безопасен observer
  const mo = new MutationObserver(() => initPuzzleGallery());
  mo.observe(document.documentElement, { childList: true, subtree: true });
})();

document.addEventListener("DOMContentLoaded", () => {
  const galleryImgs = document.querySelectorAll("#puzzleGrid img[loading='lazy']");
  if (!galleryImgs.length) return;

  // След първия layout/paint, бутаме браузъра да преоцени lazy images
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      galleryImgs.forEach((img) => {
        // 1) принудителен reflow "touch"
        img.decoding = "async";
        img.fetchPriority = "high"; // не вреди в модерни браузъри

        // 2) ако браузърът е прекалено агресивен с lazy, сменяме на eager
        img.loading = "eager";

        // 3) force reload ако има edge-case
